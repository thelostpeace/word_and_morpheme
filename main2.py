...
import time
from tools.data_loader import build_vocab_with_morpheme, build_label
...
build_vocab_with_morpheme(config.train, config.label_save, config.vocab_save)
...
writer.add_graph(model, [torch.randint(low=0, high=100, size=(config.batch_size, config.sequence_length), dtype=torch.long).to(device), torch.randint(low=0, high=100, size=(config.batch_size, config.sequence_length, config.max_ngram), dtype=torch.long).to(device), torch.randint(low=0, high=2, size=(config.batch_size, config.sequence_length, config.max_ngram), dtype=torch.long).to(device)])
print(summary(model, torch.randint(low=0, high=100, size=(config.batch_size, config.sequence_length), dtype=torch.long).to(device), torch.randint(low=0, high=100, size=(config.batch_size, config.sequence_length, config.max_ngram), dtype=torch.long).to(device), torch.randint(low=0, high=2, size=(config.batch_size, config.sequence_length, config.max_ngram), dtype=torch.long).to(device)))
...
def generate_batch(batch):
    ...
    batch_label, batch_input, batch_morpheme, batch_attention_mask = [], [], [], []
    for data in batch:
        num_input = []
        morpheme_input = []
        attention_mask = []
    ...
    tensor_label = torch.tensor(batch_label, dtype=torch.long).to(device)
    ...
    return tensor_label, tensor_input, tensor_morpheme, tensor_attention_mask
...
def test(test_data):
    ...
    model.eval()
    for i, (label, input_ids, morpheme, attention_mask) in enumerate(data):
        with torch.no_grad():
            output, _ = model(input_ids, morpheme, attention_mask)
